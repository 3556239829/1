...
<body><center>
<h2><%=LAUNCH_COMMAND %></h2>
<a href="/test/pages/hi.jsp"> Home </a>
	&nbsp;|&nbsp;
...
%></textarea>
	...
	<hr>
	<center>
		<small>jsp File Browser version <%= VERSION_NR%> revise by <a href="mailto:chenxingyu_java@sina.com">Xingyu Chen</a></small>
	</center>
	</center>
</body>
</html>
...
%>
	...
	<form class="formular" action="<%= browser_name %>" method="get" name="FileList">
	<a href="/test/pages/hi.jsp"> Home </a>
	&nbsp;|&nbsp;
	<a href="javascript:;" onclick="window.location.reload();">Refresh</a>
				...
				for (int i = 0; i < entry.length; i++) {
					...
					if (entry[i].isDirectory()) {
						...
						if (entry[i].canRead()) {
							...
							if (USE_POPUP) {
								//取文件后缀名
								String fileType = buf.substring(buf.lastIndexOf(".") + 1, buf.length());
								String[] types = {"jpg", "jpeg", "png", "bmp", "gif"};
								for (int j = 0; j < types.length; j++) {
									if(fileType.toLowerCase().equals(types[j]))
										link = "<a onmousedown=\"dis()\" href=\"" + name + "\" target=\"_blank\" title=\"非ie浏览器请拖拽\">"
												+ buf + "</a>";
									else
										link = ahref + "file=" + name + "\" target=\"_blank\">"
												+ buf + "</a>";
									break;
								}
							}
